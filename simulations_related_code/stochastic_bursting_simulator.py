...
def poisson_capture(efficiency, value):
        return np.minimum(value, np.random.poisson(efficiency * value))
...
def simulate(**sim_args):
    ...
    for steps in range(60 * 24 * days + 1):
        ...
        sampling_interval = 30 #in minutes
        if ((time_since_start >= 0) and (time_since_start % sampling_interval == 0)):
            samples[time_since_start] = state#obs_state#state
